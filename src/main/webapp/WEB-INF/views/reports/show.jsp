    ...
    <c:param name="content">
        ...
        <c:choose>
            <c:when test="${sessionScope.login_employee.id == report.employee.id}">
                <p><a href="<c:url value='?action=${actRep}&command=${commEdt}&id=${report.id}' />">この日報を編集する</a></p>
            </c:when>
            <c:otherwise>
                <p><a href="<c:url value='?action=${actRep}&command=${commLik}&id=${report.id}' />">この日報にいいねする</a></p>
            </c:otherwise>
        </c:choose>
